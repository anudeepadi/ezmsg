...
export default function EditVariablePage() {
  ...
  useEffect(() => {
    async function loadVariable() {
      try {
        const variable = await variables.get(variableId);
        setName(variable.name);
        setDisplayName(variable.display_name || '');
        setDescription(variable.description || '');
        setType(variable.type);
        setSourceType(variable.source_type);
        setDefaultValue(variable.default_value || '');
      } catch {
        addToast('error', 'Failed to load variable');
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      await variables.update(variableId, {
        name,
      ...
      const message =
        err instanceof ApiError && err.data
          ? (err.data as { detail?: string }).detail || 'Failed to update variable'
  ...
  return (
    <>
      <Header
        title="Edit Variable"
        description="Update variable configuration"
        actions={
