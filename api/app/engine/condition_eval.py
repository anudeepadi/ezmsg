...
def _preprocess(expression: str) -> str:
    ...
    Transforms AND/OR/NOT (case-insensitive) to Python and/or/not.
    """
    # Replace word-boundary AND/OR/NOT with Python equivalents
...
def _resolve_node(node: ast.AST, variables: dict[str, Any]) -> Any:
    ...
    if isinstance(node, ast.Compare):
        ...
        for op_node, comparator in zip(node.ops, node.comparators):
            ...
            # Coerce both sides for numeric comparison if possible
            ...
            # Compare same types when possible
