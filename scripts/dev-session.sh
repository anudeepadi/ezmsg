...
# Panes: API (FastAPI) | Web (Next.js) | Claude Code | Local Bash

set -euo pipefail

SESSION_NAME="ezmsg-dev"
PROJECT_DIR="/Users/vuc229/Documents/Development/Active-Projects/infrastructure/ezmsg-new"
...
launch_tmux_session() {
    ...
    tmux new-session -d -s "$SESSION_NAME" -c "${PROJECT_DIR}/api"
    tmux rename-window -t "${SESSION_NAME}" "dev"

    # Pane 0 (top-left): API (FastAPI)
    tmux send-keys -t "${SESSION_NAME}:dev" \
        "source venv/bin/activate && uvicorn app.main:app --reload --host 0.0.0.0 --port 8000" C-m

    # Split top-left horizontally → creates pane 1 (top-right)
    tmux split-window -h -t "${SESSION_NAME}:dev" -c "${PROJECT_DIR}/web"

    # Pane 1 (top-right): Web (Next.js)
